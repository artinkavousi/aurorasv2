...
// node_modules/tweakpane-plugin-infodump/dist/tweakpane-plugin-infodump.js
...
var StepConstraint = class {
  ...
  constrain(value) {
    ...
    const r2 = Math.round((value - o) / this.step);
    return o + r2 * this.step;
  }
};
...
var cn$k = ClassName("tab");
var TabView = class {
  constructor(doc, config) {
    this.element = doc.createElement("div");
    this.element.classList.add(cn$k(), bladeContainerClassName());
    config.viewProps.bindClassModifiers(this.element);
    bindValue(config.empty, valueToClassName(this.element, cn$k(void 0, "nop")));
    const titleElem = doc.createElement("div");
...
var cn$c = ClassName("col");
var ColorView = class {
  constructor(doc, config) {
    this.element = doc.createElement("div");
    this.element.classList.add(cn$c());
    config.foldable.bindExpandedClass(this.element, cn$c(void 0, "expanded"));
    bindValueMap(config.foldable, "completed", valueToClassName(this.element, cn$c(void 0, "cpl")));
...
function rgbToHsvInt(r2, g2, b) {
  ...
  const s2 = cmax === 0 ? 0 : d / cmax;
  const v = cmax;
  return [h, s2 * 100, v * 100];
}
function hsvToRgbInt(h, s2, v) {
...
function hsvToHslInt(h, s2, v) {
  const sd = 100 - Math.abs(v * (200 - s2) / 100 - 100);
  return [h, sd !== 0 ? s2 * v / sd : 0, v * (200 - s2) / (2 * 100)];
}
...
var APaletteController = class {
  ...
  handlePointerEvent_(d, opts) {
    ...
    const c = this.value.rawValue;
    const [h, s2, v] = c.getComponents("hsv");
    this.value.setRawValue(new IntColor([h, s2, v, alpha], "hsv"), opts);
  }
  onPointerDown_(ev) {
    this.handlePointerEvent_(ev.data, {
      forceEmit: false,
  ...
  onKeyDown_(ev) {
    ...
    const c = this.value.rawValue;
    const [h, s2, v, a] = c.getComponents("hsv");
    this.value.setRawValue(new IntColor([h, s2, v, a + step], "hsv"), {
      forceEmit: false,
      last: false
    });
  }
...
function createHexController(doc, config) {
  ...
  connectValues({
    ...
    backward: (p2, s2) => new IntColor(appendAlphaComponent(removeAlphaComponent(s2.getComponents(p2.mode)), p2.getComponents()[3]), p2.mode)
  });
  return [c];
}
...
var HPaletteController = class {
  ...
  handlePointerEvent_(d, opts) {
    ...
    const c = this.value.rawValue;
    const [, s2, v, a] = c.getComponents("hsv");
    this.value.setRawValue(new IntColor([hue, s2, v, a], "hsv"), opts);
  }
  onPointerDown_(ev) {
    this.handlePointerEvent_(ev.data, {
      forceEmit: false,
  ...
  onKeyDown_(ev) {
    ...
    const c = this.value.rawValue;
    const [h, s2, v, a] = c.getComponents("hsv");
    this.value.setRawValue(new IntColor([h + step, s2, v, a], "hsv"), {
      forceEmit: false,
      last: false
    });
  }
  ...
  update_() {
    ...
    for (let iy = 0; iy < height; iy++) {
      for (let ix = 0; ix < width; ix++) {
        ...
        const rgbComps = hsvToRgbInt(hsvComps[0], s2, v);
...
var SvPaletteController = class {
  ...
  onKeyDown_(ev) {
    ...
    const [h, s2, v, a] = this.value.rawValue.getComponents("hsv");
    ...
    this.value.setRawValue(new IntColor([h, s2 + ds, v + dv, a], "hsv"), {
      forceEmit: false,
      last: false
    });
  }
...
var cn$4 = ClassName("p2d");
var Point2dView = class {
  constructor(doc, config) {
    this.element = doc.createElement("div");
    this.element.classList.add(cn$4());
    config.viewProps.bindClassModifiers(this.element);
    bindValue(config.expanded, valueToClassName(this.element, cn$4(void 0, "expanded")));
    const headElem = doc.createElement("div");
...
var e = [];
var r = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" };
var l = new RegExp(`[${Object.keys(r).join("")}]`, "g");
var g = (t2) => t2.replace(l, (t3) => r[t3]);
var p = [[/\r\n/g, "\n"], [/\n(#+)(.*)/g, (t2, n2, e2 = "") => {
  const r2 = n2.length;
  return `<h${r2}>${e2.trim()}</h${r2}>`;
}], [/!\[([^\[]+)\]\((?:javascript:)?([^\)]+)\)/g, '<img src="$2" alt="$1">'], [/\[([^\[]+)\]\((?:javascript:)?([^\)]+)\)/g, '<a href="$2">$1</a>'], [/([^\\])(\*\*|__)(.*?(_|\*)?)\2/g, "$1<strong>$3</strong>"], [/([^\\])(\*|_)(.*?)\2/g, "$1<em>$3</em>"], [/\\_/g, "&#95;"], [/\~\~(.*?)\~\~/g, "<del>$1</del>"], [/\:\"(.*?)\"\:/g, "<q>$1</q>"], [/\n( *)(\*|-|\+)(.*)/g, (t2, n2, e2, r2) => {
  const l2 = 1 + Math.floor(n2.length / 2);
  return "\n<ul>".repeat(l2) + "\n	<li>" + r2.trim() + "</li>" + "\n</ul>".repeat(l2);
}], [/\n( *)([0-9]+\.)(.*)/g, (t2, n2, e2, r2) => {
  const l2 = 1 + Math.floor(n2.length / 2);
  return "\n<ol>".repeat(l2) + "\n	<li>" + r2.trim() + "</li>" + "\n</ol>".repeat(l2);
}], [/\n(&gt;|\>)(.*)/g, (t2, n2, e2 = "") => `
<blockquote>${e2.trim()}</blockquote>`], [/(\^)(.*?)\1/g, "<sup>$2</sup>"], [/(\~)(.*?)\1/g, "<sub>$2</sub>"], [/\n-{5,}/g, "\n<hr />"], [/( *\|[^\n]+\|\r?\n)((?: *\|:?[ -]+:?)+ *\|)(\n(?: *\|[^\n]+\|\r?\n?)*)?/g, (t2, n2, e2, r2) => {
  const l2 = e2.split("|").filter((t3, n3, e3) => n3 > 0 && n3 < e3.length - 1).map((t3) => /:-+:/g.test(t3) ? "center" : /-+:/g.test(t3) ? "right" : /:-+/.test(t3) ? "left" : ""), g2 = (t3) => {
    const n3 = l2[t3];
    return n3 ? ` align="${n3}"` : "";
  };
  return `
<table><tbody><tr>${n2.split("|").map((t3) => t3.trim()).filter((t3) => t3 && t3.length).map((t3, n3) => `<th${g2(n3)}>${t3}</th>`).join("")}</tr>${r2.split("\n").map((t3) => t3.trim()).filter((t3) => t3 && t3.length).map((t3) => `<tr>${t3.split("|").filter((t4, n3, e3) => n3 > 0 && n3 < e3.length - 1).map((t4, n3) => `<td${g2(n3)}>${t4.trim()}</td>`).join("")}</tr>`).join("")}</tbody></table>
`;
}], [/\[\^([^\]]+)\](?!:)/g, (t2, n2) => `<sup id="fnref:${n2}"><a href="#fn:${n2}">[${n2}]</a></sup>`], [/\[\^([^\]]+)\]:\s*((?:[^\n]*\n?)*)/g, (t2, n2, r2) => (e.push([n2, r2.trim()]), "")], [/\n([^\n]+)\n/g, (t2, n2) => {
  const e2 = n2.trim();
  return /^<\/?(ul|ol|li|h|p|bl|table|tr|td)/i.test(e2) ? `
${n2}
` : `
<p>
${e2}
...
`;
}], [/\s?<\/[ou]l>\s?<[ou]l>/g, "", 3], [/<\/blockquote>\n<blockquote>/g, "<br>\n"], [/https?:\/\/[^"']*/g, (t2) => t2.replace(/<\/?em>/g, "_")], [/&#95;/g, "_"]];
var s = (r2, l2 = false, s2 = false) => (t.length = 0, n.length = 0, e.length = 0, r2 = ((t2) => t2.replace(/`([^`]+)`/g, (t3, e2) => (n.push(e2), `{{INLINECODEPH${n.length - 1}}}`)))(r2 = ((n2) => n2.replace(/\n\s*```\w*\n([^]*?)\n\s*```\s*\n/g, (n3, e2) => (t.push(e2), `
<pre>{{CODEBLOCKPH${t.length - 1}}}</pre>
`)))(`
${r2}
`)), p.forEach(([t2, n2, e2 = 1]) => {
  for (let l3 = 0; l3 < e2; l3++) r2 = r2.replace(t2, n2);
}), r2 = (r2 = ((t2) => t2.replace(/{{INLINECODEPH(\d+)}}/g, (t3, e2) => {
  const r3 = n[parseInt(e2)];
  return `<code>${g(r3)}</code>`;
}))(r2 = ((n2) => n2.replace(/<pre>{{CODEBLOCKPH(\d+)}}<\/pre>/g, (n3, e2) => {
  const r3 = t[parseInt(e2)];
  return `<pre>${g(r3)}</pre>`;
}))(r2))).trim() + (0 === e.length ? "" : `
<div class="footnotes">
  <hr>
  <ol>
    ...
    <li id="fn:${t2}">
      ${n2}
      <sup><a href="#fnref:${t2}">↩</a></sup>
    </li>`).join("\n")}
  </ol>
</div>`), l2 && (r2 = r2.replace(/^<p>([\s\S]*)<\/p>$/, "$1")), s2 && (r2 = r2.replace(/<a href="/g, '<a target="_blank" href="')), r2);
var className = ClassName("indu");
var InfodumpView = class {
  constructor(doc, config) {
    this.element = doc.createElement("div");
    ...
    if (config.markdown) {
      contentElem.innerHTML = s(config.content);
    } else {
...
var InfodumpController = class extends BladeController {
  constructor(doc, config) {
    super({
      blade: createBlade(),
...
var TweakpaneInfodumpPlugin = createPlugin({
  id: "infodump",
  ...
  type: "blade",
  accept(params) {
    const r2 = parseRecord(params, (p2) => ({
      content: p2.required.string,
      markdown: p2.optional.boolean,
      view: p2.required.constant("infodump")
    }));
  ...
  controller(args) {
    ...
    return new InfodumpController(args.document, {
      ...
      markdown: (_a = args.params.markdown) !== null && _a !== void 0 ? _a : false,
      viewProps: args.viewProps
    });
  },
  api(args) {

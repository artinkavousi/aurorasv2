...
## 1. Deep Analysis
- **Current structure overview.** The runtime is orchestrated by `app.js`, which owns renderer setup, camera/controls, HDRI loading, post-processing, MLS-MPM simulation lifecycle, and input handling in a single class, while other files provide utilities for lights, background geometry, simulation buffers, and UI controls.【F:src/app.js†L1-L173】【F:src/lights.js†L1-L25】【F:src/backgroundGeometry.js†L1-L73】【F:src/mls-mpm/mlsMpmSimulator.js†L1-L400】【F:src/conf.js†L1-L121】【F:src/info.js†L1-L48】 Assets are colocated under `src/assets`, and MLS-MPM helpers sit in `src/mls-mpm` with shared math in `src/common`.
...
- **Interface gaps & tight coupling.** Most modules accept global singletons (e.g., `conf`, `Info`, `Lights`) and reach into internal buffers directly instead of consuming clear APIs, preventing reuse or hot-swapping and binding everything to `app.js` construction order.【F:src/app.js†L42-L170】【F:src/mls-mpm/particleRenderer.js†L122-L178】【F:src/mls-mpm/mlsMpmSimulator.js†L371-L397】 There are no explicit lifecycle hooks beyond implicit constructors.
- **Performance & DX pain points.** `app.js` performs sequential async loads for HDRI, geometry, and simulation with ad-hoc progress callbacks instead of structured asset management; resizing only touches the camera but not the post stack, and bloom toggles require branching per frame.【F:src/app.js†L65-L168】 The MLS-MPM simulator is a 400+ line monolith without separation between data schema, compute kernels, and frame update, complicating profiling or worker offloading.【F:src/mls-mpm/mlsMpmSimulator.js†L1-L400】
...
- `src/commons/assets.ts` — Central asset registry, async loaders for HDRI, OBJ, textures, with progress events to replace ad-hoc `RGBELoader` calls.【F:src/app.js†L65-L118】【F:src/backgroundGeometry.js†L17-L61】 **Exports:** `createAssetStore()`, `loadAsset(id)`, `preload(list)`. **Hooks:** handles `init/dispose` for loaders and caches.
...
- `src/stage/stage.ts` — Owns renderer, camera, controls, environment maps, resize, pointer events; exposes `StageHandle` and pointer ray emitter instead of `app.js` monolith.【F:src/app.js†L42-L170】
- `src/stage/lights.ts` — Contains light rig creation/updater with parameterized presets, factoring out `Lights` class for reuse.【F:src/lights.js†L1-L25】
...
- `src/postfx/postfx.ts` — Wraps `THREE.PostProcessing`, bloom, tone mapping, and MRT integration with parameterized controls.【F:src/app.js†L97-L168】
